...
- `inventory.ts` — one record per linked worktree of a configured repo, assembled from the git module and the repo's config (naming via `branch.ts`, managed flag and `id` via the `plan.ts` placement the worktree stands on, so a disambiguated path reads as conventional and under the name `up` gave it). Assembles only; `ls` renders and `prune` (#27) will act. Herdr facts enter through `attachTabFacts()` as plain data, so the module keeps the layering rule (worktree/ never imports herdr/) while `ls` merges pane snapshots into the records.
