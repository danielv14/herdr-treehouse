...
- `inventory.ts` — one record per linked worktree of a configured repo, assembled from the git module and the repo's config (naming via `branch.ts`, managed flag via `plan.ts` path match). Assembles only; `ls` renders and `prune` (#27) will act. Herdr facts enter through `attachTabFacts()` as plain data, so the module keeps the layering rule (worktree/ never imports herdr/) while `ls` merges pane snapshots into the records.
...
- `git.ts` — the only module that knows git, the way `tabs.ts` is the only one that knows Herdr: argv, ref shapes, and the flags that must NOT be there. Commands ask questions (`inspectCheckout`: root, main root, linked, dirty in one call; `findWorktreeForBranch`: which worktree holds a branch, main checkout included so the caller can refuse it) and name intents (`addWorktree` with the branch-exists and base-refresh policy inside, `removeWorktree` without `--force`, pinned by `git.test.ts` against real repos).
