...
- `plan.ts` — everything derived about one worktree (path, base ref, slug/ticket/id, placeholder expansion) from a single `buildWorktreePlan()` call. Pure: no fs, no git, no Herdr. An unknown `{placeholder}` is an error, not a pass-through. `bootstrapTakesTargets()` answers whether asking for targets makes sense; the `{targets...}` placeholder itself is private. `worktreePlacements()` is the other half of #32: the ordered `{id, worktree}` spots the convention allows one branch, so the caller (which is the only one that can ask git what is taken) picks one and hands it back through `worktree` + `id`.
- `inventory.ts` — one record per linked worktree of a configured repo, assembled from the git module and the repo's config (naming via `branch.ts`, managed flag and `id` via the `plan.ts` placement the worktree stands on, so a disambiguated path reads as conventional and under the name `up` gave it). Assembles only; `ls` renders, and nothing acts on the records (a `prune` command was specced and dropped, see below). Herdr facts enter through `attachTabFacts()` as plain data, so the module keeps the layering rule (worktree/ never imports herdr/) while `ls` merges pane snapshots into the records.
- `provision.ts` — "make this worktree exist and be usable", shared by `up` and the `worktree.created` hook. `worktreeState: 'just-created'` is how the hook says Herdr already made the checkout but it is still fresh, so `setup` must run, and `setupExisting` is how `up --setup` says the opposite: the worktree is old but setup belongs there anyway. Setup expands its whole command list before running any of it, so a typo'd placeholder stops the run before the first command has changed the worktree.
- `git.ts` — the only module that knows git, the way `tabs.ts` is the only one that knows Herdr: argv, ref shapes, and the flags that must NOT be there. Commands ask questions (`inspectCheckout`: root, main root, linked, dirty in one call; `findWorktreeForBranch`: which worktree holds a branch, main checkout included so the caller can refuse it; `findWorktreeAtPath`: which branch occupies a path, the question `existsSync` answers wrong, sharing the module's `samePath` so git's spelling of a path and a derived one compare equal) and name intents (`addWorktree` with the branch-exists and base-refresh policy inside, `removeWorktree` without `--force`, pinned by `git.test.ts` against real repos).
...
- **Sidebar token (`$worktrees`, issue #26 MVP)**: `treehouse report` pushes the per-repo worktree count as workspace metadata. Freshness comes from three sides: `up`/`down` report in-process after changing the count (Herdr's worktree events see only its native flow, not our git-side changes), the manifest's `worktree.created/removed` hooks cover the native flow, and `[[startup]]` re-reports after a server restart (tokens are not persisted). Per-worktree tokens (`$ticket`, `$dirty`) and PR enrichment were deliberately cut from the MVP: dirty/ahead-behind change on git activity, which no hookable event sees, so their sidebar freshness story is unsolved (noted in #26).
