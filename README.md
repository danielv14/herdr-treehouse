...
Half-configured is an error rather than silence, the same way a typo'd placeholder is: `context` with no `{context_file}` to read it is refused, and `{context_file}` with no `context` resolved is refused. `--no-agent` needs neither and writes no file. All of it is checked before the worktree is provisioned.
